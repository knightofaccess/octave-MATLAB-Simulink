...
alpha2G=10;
alpha3G=1;
u_ref=0;gama_ref=pi/60;GAMA_ref=0;
alpha1f=2.5e-7;
alpha2f=0.025;
alpha3f=1;

% Glide slope
ap=[Xu Xw 0 -g;Zu Zw Zq 0;Mu Mw Mq 0;0 0 1 0]
% bn=[Xn;Zn;Mn;0];
bp=[XL XT XR;ZL ZT ZR;ML MT MR;0 0 0]
Cpg=[1 0 0 0 0 0 0;0 1/U0 0 -1 0 0 0]; Dp=[0 0 0;0 0 0];
np=4;% no of original plant states
ncp=np+ncon; % No of plant states + number of engines (controls)
ngs=3; % No of additional GS kinematic states
...
% Plant: aircraft and Glide path kinematics
% act=[3.91e-06 -1.77e-07 7e-08 0]'; % boeing
% A11=[ap act;zeros(1,4) -pthrot] % Boeing
A11=[ap bp; zeros(ncon,np) -pthrot*eye(ncon)]
A12=zeros(ncp,ngs)
A211=[-1/T1 0 0 0;0 -Vc0/U0 0 Vc0 ;zeros(1, np)]
A21=[A211 zeros(ncon, ngs)]
A22=[-1/T1 0 0;0 0 0;0 -1/Rn0 0]
B11=zeros(ncp,ndis);
B12=[zeros(np,ncon);pthrot*eye(ncon)];
B21=[[1/T1 0 0; 0 Vc0 0 ; 0 0 1] 0*eye(noisem)];
B22=zeros(ngs,ncon);
%Criterion function
C11=zeros(ncr,ncp);
C12=[alpha1G 0 0;0 0 alpha2G;0 0 0; 0 0 0; 0 0 0];
D11=zeros(ncr,ndis);
D12=[0 0 0;0 0 0; alpha3G*eye(ncon)];
...
Dg=[D11 D12;D21 D22]
%gnormz=10000;
%rg=Bg2*inv(D12'*D12)*Bg2'-(Bg1*Bg1')/(gnormz^2)
%qg=Cg1'*Cg1
%X=are(Ag, rg, qg
if rdef==0
P=ss(Ag, Bg  , Cg , Dg);
[Kgs,Cloop2,gnormz] = h2syn(P,nmeas,ncon);
else
